...
#[Group('Admin')]
#[Endpoint('Process Payout', 'Mark an approved payout as processing')]
#[Authenticated]
#[Header('Authorization', 'Bearer <token>')]
#[UrlParam(name: 'payout', type: 'string', description: 'ULID of the payout.', required: true)]
#[Response(status: 200, description: 'Payout is now processing.')]
#[Response(status: 422, description: 'Payout is not in approved status.')]
final class ProcessPayoutController extends Controller
{
    ...
    public function __invoke(DriverPayout $payout): JsonResponse
    {
        ...
        $payout = $this->action->process($payout);

        return ApiResponse::success(
            AdminPayoutData::fromModel($payout, $this->avatarResolver),
            __('messages.payout.processing'),
        );
    }
}
