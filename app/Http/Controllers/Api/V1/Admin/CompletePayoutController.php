...
#[Group('Admin')]
#[Endpoint('Complete Payout', 'Mark a processing payout as completed')]
#[Authenticated]
#[Header('Authorization', 'Bearer <token>')]
#[UrlParam(name: 'payout', type: 'string', description: 'ULID of the payout.', required: true)]
#[Response(status: 200, description: 'Payout completed successfully.')]
#[Response(status: 422, description: 'Payout is not in processing status.')]
final class CompletePayoutController extends Controller
{
    ...
    public function __invoke(DriverPayout $payout): JsonResponse
    {
        ...
        $payout = $this->action->complete($payout);

        return ApiResponse::success(
            AdminPayoutData::fromModel($payout, $this->avatarResolver),
            __('messages.payout.completed'),
        );
    }
}
