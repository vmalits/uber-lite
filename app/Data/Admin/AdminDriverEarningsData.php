<?php

declare(strict_types=1);

namespace App\Data\Admin;

use Spatie\LaravelData\Attributes\MapName;
use Spatie\LaravelData\Data;

final class AdminDriverEarningsData extends Data
{
    /**
     * @param array<int, array{date: string, total_rides: int, total_earnings: int, average_per_ride: float}> $daily
     */
    public function __construct(
        #[MapName('total_earned')]
        public float $totalEarned,
        #[MapName('total_rides')]
        public int $totalRides,
        #[MapName('completed_rides')]
        public int $completedRides,
        #[MapName('cancelled_rides')]
        public int $cancelledRides,
        #[MapName('completion_rate')]
        public float $completionRate,
        #[MapName('average_earnings_per_ride')]
        public float $averageEarningsPerRide,
        #[MapName('average_rating')]
        public float $averageRating,
        #[MapName('total_payouts')]
        public int $totalPayouts,
        #[MapName('pending_payouts')]
        public int $pendingPayouts,
        /** @var array<int, array{date: string, total_rides: int, total_earnings: int, average_per_ride: float}> */
        #[MapName('daily')]
        public array $daily,
    ) {}
}
