...
test('driver can get streak history', function (): void {
    ...
    $response = $this->getJson('/api/v1/driver/streak/history');
...
test('driver can get streak history with custom days parameter', function (): void {
    ...
    $response = $this->getJson('/api/v1/driver/streak/history?days=7');
...
test('driver can get streak history with no streak data', function (): void {
    Sanctum::actingAs($this->driver);
...
test('unauthorized user cannot get driver streak history', function (): void {
    $response = $this->getJson('/api/v1/driver/streak/history');

    $response->assertUnauthorized();
});

test('rider cannot access driver streak history', function (): void {
    ...
    $this->getJson('/api/v1/driver/streak/history')
        ->assertForbidden();
});
...
test('validation fails for invalid days parameter', function (): void {
    ...
    $response = $this->getJson('/api/v1/driver/streak/history?days=5');
...
test('validation fails for days parameter exceeding max', function (): void {
    ...
    $response = $this->getJson('/api/v1/driver/streak/history?days=100');
