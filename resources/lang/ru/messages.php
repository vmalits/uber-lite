<?php

declare(strict_types=1);

return [
    'auth' => [
        'phone_required'           => 'Номер телефона обязателен',
        'phone_invalid'            => 'Номер телефона недействителен',
        'otp_sent'                 => 'Код OTP успешно отправлен',
        'otp_expired'              => 'Код OTP истёк',
        'otp_invalid'              => 'Неверный код OTP',
        'verification_failed'      => 'Ошибка верификации',
        'unauthorized'             => 'Не авторизован',
        'user_not_found'           => 'Пользователь не найден',
        'logged_out'               => 'Выход успешно выполнен.',
        'account_deleted'          => 'Аккаунт успешно удалён.',
        'email_added'              => 'Email успешно добавлен.',
        'phone_not_verified'       => 'Номер телефона не подтверждён.',
        'role_selected'            => 'Роль успешно выбрана.',
        'profile_completed'        => 'Профиль успешно заполнен.',
        'otp_requested'            => 'OTP был успешно запрошен.',
        'otp_resent'               => 'OTP был успешно отправлен повторно.',
        'email_verify_link'        => 'Недействительная ссылка верификации.',
        'email_already_verified'   => 'Ваш email уже подтверждён.',
        'email_verified'           => 'Email успешно подтверждён.',
        'verification_link_sent'   => 'Ссылка для верификации была отправлена.',
        'too_many_attempts'        => 'Слишком много неудачных попыток верификации. Пожалуйста, запросите новый OTP.',
        'phone_already_verified'   => 'Номер телефона уже подтверждён.',
        'verification_successful'  => 'Верификация прошла успешно.',
        'invalid_or_expired_code'  => 'Неверный или истёкший код.',
        'phone_not_verified_error' => 'Номер телефона не подтверждён.',
        'role_already_selected'    => 'Роль уже выбрана.',
        'email_not_set'            => 'Email не установлен.',
    ],
    'validation' => [
        'required' => 'Поле :attribute обязательно для заполнения',
        'email'    => 'Поле :attribute должно быть действительным email',
        'min'      => 'Поле :attribute должно содержать минимум :min символов',
        'max'      => 'Поле :attribute не может содержать более :max символов',
    ],
    'errors' => [
        'server_error'      => 'Ошибка сервера',
        'not_found'         => 'Ресурс не найден',
        'forbidden'         => 'Доступ запрещён.',
        'invalid_locale'    => 'Неверная локаль',
        'unauthorized'      => 'Не авторизован.',
        'validation_failed' => 'Данные недействительны.',
        'too_many_requests' => 'Слишком много запросов.',
    ],
    'success' => [
        'created'        => 'Успешно создано',
        'updated'        => 'Успешно обновлено',
        'deleted'        => 'Успешно удалено',
        'locale_updated' => 'Локаль успешно обновлена',
    ],
    'ride' => [
        'created'              => 'Поездка успешно создана',
        'scheduled'            => 'Поездка успешно запланирована',
        'accepted'             => 'Поездка успешно принята',
        'completed'            => 'Поездка успешно завершена',
        'cancelled'            => 'Поездка успешно отменена',
        'rated'                => 'Поездка успешно оценена',
        'shared'               => 'Поездка успешно отправлена',
        'split'                => 'Поездка успешно разделена',
        'not_active'           => 'Активная поездка не найдена',
        'already_active'       => 'У вас уже есть активная поездка.',
        'estimate_calculated'  => 'Оценка рассчитана успешно.',
        'pin_verified'         => 'PIN-код подтверждён.',
        'pin_already_verified' => 'PIN-код уже был подтверждён.',
        'tip_added'            => 'Чаевые успешно добавлены.',
        'message_sent'         => 'Сообщение отправлено.',
    ],
    'profile' => [
        'updated' => 'Профиль успешно обновлён',
    ],
    'avatar' => [
        'upload_started' => 'Загрузка аватара началась',
    ],
    'driver' => [
        'online'                   => 'Теперь вы онлайн и доступны для поездок.',
        'offline'                  => 'Теперь вы офлайн.',
        'earnings_fetched'         => 'Дневной заработок успешно получен.',
        'weekly_earnings_fetched'  => 'Недельный заработок успешно получен.',
        'monthly_earnings_fetched' => 'Месячный заработок успешно получен.',
    ],
    'favorite' => [
        'location_added'   => 'Избранное место успешно добавлено.',
        'location_removed' => 'Избранное место успешно удалено.',
        'driver_added'     => 'Избранный водитель успешно добавлен.',
        'driver_removed'   => 'Избранный водитель успешно удален.',
    ],
    'vehicle' => [
        'added' => 'Транспортное средство успешно добавлено.',
    ],
    'payment_methods' => [
        'fetched' => 'Способы оплаты успешно получены.',
    ],
    'safety' => [
        'contact_added'         => 'Контакт для экстренных случаев успешно добавлен.',
        'contact_deleted'       => 'Контакт для экстренных случаев успешно удалён.',
        'sos_sent'              => 'SOS-сигнал отправлен вашим контактам для экстренных случаев.',
        'no_emergency_contacts' => 'Контакты для экстренных случаев не найдены. Пожалуйста, сначала добавьте контакты.',
    ],
    'streak' => [
        'level_up_subject'  => 'Поздравляем! Ваш уровень серии повысился!',
        'level_up_greeting' => 'Отличные новости, :name!',
        'level_up_message'  => 'Вы достигли уровня :level, совершив поездки :streak дней подряд! Теперь вы получаете скидку :discount% на все поездки.',
        'keep_going'        => 'Продолжайте ездить каждый день, чтобы получить ещё большие скидки!',
    ],
    'payout' => [
        'requested'  => 'Запрос на выплату успешно отправлен.',
        'approved'   => 'Выплата одобрена.',
        'processing' => 'Выплата обрабатывается.',
        'completed'  => 'Выплата успешно завершена.',
        'failed'     => 'Выплата отклонена.',
    ],
    'payment_method' => [
        'deleted'     => 'Платёжный метод успешно удалён.',
        'set_default' => 'Платёжный метод установлен по умолчанию.',
        'added'       => 'Платёжный метод успешно добавлен.',
    ],
    'payment' => [
        'completed'            => 'Оплата успешно обработана.',
        'failed'               => 'Оплата не удалась.',
        'refunded'             => 'Оплата успешно возвращена.',
        'status_retrieved'     => 'Статус оплаты успешно получен.',
        'insufficient_credits' => 'Недостаточно кредитов на балансе.',
    ],
    'notifications' => [
        'marked_as_read'     => 'Уведомление отмечено как прочитанное.',
        'all_marked_as_read' => 'Все уведомления отмечены как прочитанные.',
        'deleted'            => 'Уведомление удалено.',
        'not_found'          => 'Уведомление не найдено.',
    ],
    'announcement' => [
        'created' => 'Объявление успешно создано.',
        'updated' => 'Объявление успешно обновлено.',
        'deleted' => 'Объявление успешно удалено.',
    ],
    'device' => [
        'registered' => 'Токен устройства успешно зарегистрирован.',
        'deleted'    => 'Токен устройства успешно удалён.',
    ],
    'report' => [
        'created'  => 'Жалоба успешно создана.',
        'resolved' => 'Жалоба успешно обновлена.',
    ],
    'achievement' => [
        'created' => 'Достижение успешно создано.',
        'updated' => 'Достижение успешно обновлено.',
        'deleted' => 'Достижение успешно удалено.',
    ],
    'document' => [
        'uploaded' => 'Документ успешно загружен.',
        'verified' => 'Документ успешно проверен.',
    ],
    'pricing_zone' => [
        'created' => 'Зона ценообразования успешно создана.',
        'updated' => 'Зона ценообразования успешно обновлена.',
        'deleted' => 'Зона ценообразования успешно удалена.',
    ],
    'user' => [
        'status_updated' => 'Статус пользователя успешно обновлён.',
    ],
];
